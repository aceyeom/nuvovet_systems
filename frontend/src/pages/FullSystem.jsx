              ...
              return (
                ...
                >
                  <div className="flex items-start gap-2.5">
                    <span className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[9px] font-bold tracking-[0.14em] ${
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        <p className={`text-[13px] ${isActive ? 'font-semibold text-slate-900' : 'font-medium text-slate-700'}`}>
                          {stage.title}
                        </p>
                        <span className={`rounded-full border px-2 py-0.5 text-[9px] font-semibold tracking-wide ${stage.statusTone}`}>
                          {stage.status}
                        </span>
                      </div>
            ...
            return (
              <div key={drug.id} className={`px-3 py-2.5 ${idx !== drugs.length - 1 ? 'border-b border-slate-100' : ''}`}>
                <div className="flex items-center gap-2">
                  <p className="text-[12px] font-semibold text-slate-900 truncate">{drug.nameKr || drug.name}</p>
                  {drug.nameKr && drug.name && <span className="text-[10px] text-slate-400 truncate">{drug.name}</span>}
                  {dosePerKg > 0 && totalMg && (
                ...
                {drug.productNamesKo?.length > 0 && (
                  <div className="flex flex-wrap gap-0.5 mt-0.5">
                    {drug.productNamesKo.slice(0, 2).map(p => (
                      <span key={p} className="text-[8px] px-1 py-0.5 bg-blue-50 text-blue-500 rounded border border-blue-100 truncate max-w-[100px]">{p}</span>
                    ))}
                    {drug.productNamesKo.length > 2 && (
                      <span className="text-[8px] text-slate-400">+{drug.productNamesKo.length - 2}</span>
                    )}
                  </div>
                )}
        ...
        {step === 'input' && (
          <div className="flex-1 overflow-hidden flex flex-col xl:flex-row">
            ...
            >
              ...
              {/* ── NEW PATIENT form ── */}
              {patientTab === 'new' && (
                <div className="px-5 py-4 sm:px-6 sm:py-5 space-y-3.5">
            ...
            <div
              ...
              className="w-full overflow-y-auto bg-white min-h-0 xl:border-l border-slate-200"
            >
